...
class Compiler(object):
    ...
    def compile(output_view):
        ...
        compile_command = compile_template.substitute(code_file_extension=Environment.file_extension,
                                                      code_file_base_name=Environment.file_base_name,
        ...
        output_view.run_command("output_file_edit",
                                {"append": "Compilation finished with exit code : " + str(compile.returncode)});
...
class Executor(object):
    ...
    def execute(input_view, output_view):
        ...
        execute_command = execute_template.substitute(code_file_extension=Environment.file_extension,
                                                      code_file_base_name=Environment.file_base_name,
                                                      code_file_path=Environment.file_path, code_file=Environment.file)
...
class CompileAndRunCommand(sublime_plugin.TextCommand):
    def run(self, edit):
        Environment.last_runnable_view.run_command('save')
        input_view = Environment.last_input_view
        output_view = Utils.create_output_file()
        if None == input_view:
            output_view.run_command("output_file_edit",
                                    {"append": "No input file selected (Move focus to an input file)"});
            return None
